<template>
  ...
  >
    <a-grid :cols="18" :col-gap="12" :row-gap="12">
...
<script setup lang="ts">
  ...
  // import { TrendsDetail,TimeDistributionItem } from '@/api/visualization';
  import ChainItem from './chain-item.vue';

  const { isDark } = useThemes();

  //   const recentVideo = ref(0);
  //   const tlikes = ref(0);
  //   const tshares = ref(0);

  //   onMounted(async () => {
  //     const res = await TrendsDetail();
  //     const {
  //       recent_seven_days_videos: recentSevenDaysVideos,
  //       total_likes: totalLikes,
  //       total_shares: totalShares,
  // } = res.data.data;
  //     recentVideo.value = recentSevenDaysVideos;
  //     tlikes.value = totalLikes;
