...
from webs.config import DATABASE_URI
...
class MaliciousUser(Base):
    """恶意用户信息表"""

    __tablename__ = "malicious_users"  # 表名

    id = Column(Integer, primary_key=True)  # 自增主键
    user_id = Column(String(255), unique=True, comment="用户ID（唯一标识）")
    user_name = Column(String(255), comment="用户名")
    age = Column(String(50), comment="年龄（可能为空）")
    follow_count = Column(Integer, comment="关注数")
    fans_count = Column(Integer, comment="粉丝数")
    like_count = Column(Integer, comment="获赞数")
    douyin_id = Column(String(255), comment="抖音ID")
    ip_location = Column(String(255), comment="IP所在地")
    self_description = Column(Text, comment="个人简介（可能为空）")
